...
var jsonChart;
  ...
  $.ajax({
      ...
      dataType: "json",
      url: "partials/dgm/TransferenciasEntidadesFederativas.json",
  ...
  var valor = obtenerValorParametro("muestra");
    if (valor == "td"){
      ...
      dimension = window.innerHeight - $("#titulo").outerHeight(true) - $("#descripcion").outerHeight(true) - $("#vermas").outerHeight(true) - 60;
    }
    else if (valor == "t"){
      $('#descripcion').remove();
      $("#titulo").html("<strong><p>"+jsonChart.titulo+"</p></strong>");
